...
async function callWithRetry(fn, intentos = 3, espera = 2000) {
  for (let i = 0; i < intentos; i++) {
    try {
      return await fn();
    } catch (error) {
      const esOverloaded = error?.status === 529 || error?.message?.includes("overloaded");
      const esUltimo = i === intentos - 1;
      if (esOverloaded && !esUltimo) {
...
{"recetas":[{"nombre":"nombre","emoji":"🍳","tiempo":"20 min","dificultad":"Fácil","porciones":"2","ingredientes":["item1","item2"],"pasos":["paso1","paso2"],"beneficios":["beneficio1","beneficio2"]}]}`;
    ...
    const mensaje = esOverloaded
      ? "El servicio está muy ocupado en este momento. Intentá de nuevo en unos segundos."
      : "Ocurrió un problema al procesar tu solicitud. Intentá de nuevo.";
